...
Ptr<OutputStreamWrapper> bytesRxStream;
uint32_t cWndValue;
...
CwndTracer (uint32_t oldval, uint32_t newval)
{
  ...
  *cWndStream->GetStream () << Simulator::Now ().GetSeconds () << " " << newval << std::endl;
  cWndValue = newval;
    ...
    double cur = (sink->GetTotalRx ()) * (double) 8 / Simulator::Now().GetSeconds()/1000 /1000;
    
    *throughputStream->GetStream () << Simulator::Now().GetSeconds() << " " << cur <<std::endl;

    *bytesRxStream->GetStream () << Simulator::Now().GetSeconds() << " " << sink->GetTotalRx () <<std::endl;
  ...
  if (tracing)
    {
      ...
      //stack.EnableAsciiIpv4All (ascii_wrap);
        ...
        Simulator::Schedule (Seconds (0.00001), &TraceQueue, prefix_file_name + "-queue.data");
    }
